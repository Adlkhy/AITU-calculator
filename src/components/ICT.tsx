...
function Programming() {
  ...
  useEffect(() => {
    ...
    // --- RegMid Calculation ---
    const avgAssignments1 = (((p(assignment1) + p(assignment2)) / 100) * 25) + (((p(quiz) + p(learn)) / 100) * 10);
    const midtermScore = (p(midtermQuiz) / 100) * 30;
    const calculatedAtt1 = (avgAssignments1) + (midtermScore);
    setAttestation1(calculatedAtt1);

    // --- RegEnd Calculation ---
    const avgAssignments2 = (((p(assignment3) + p(assignment4)) / 100) * 25) + (((p(quiz2) + p(learn2)) / 100) * 10);
    const endtermScore = (p(endtermQuiz) / 100) * 30;
    ...
    assignment1, assignment2, quiz, learn, midtermQuiz,
    assignment3, assignment4, quiz2, learn2, endtermQuiz,
    finalExam
  ]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <Card className="p-5">
              <CardContent className='px-0'>
                ...
                <GradeInput label="Learn" value={learn} onChange={setLearn} />
            ...
            <div>
                <Card className="p-5 mb-8">
                  <CardContent className='px-0'>
                    ...
                    <GradeInput label="Learn" value={learn2} onChange={setLearn2} />
                    <hr className="border-foreground my-4" />
