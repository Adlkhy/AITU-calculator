                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-background border border-muted-foreground/20 p-2 rounded-lg shadow-xl text-[10px] font-bold uppercase tracking-widest">
                        ...
                        <p className="text-muted-foreground font-mono">{payload[0].payload.amount.toFixed(0)}<span className='font-sans'>₸</span></p>
                      </div>
                    );
                  }
                  return null;
        ...
        <div className="space-y-2">
          {chartData.map((item) => (
            <div key={item.name} className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold font-mono">{item.amount.toLocaleString()}<span className='font-sans'>₸</span></span>
            </div>
          ))}
        </div>
      </CardContent>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-muted/30 p-2 rounded-2xl border border-muted-foreground/10">
          ...
          <div className="pr-4">
            ...
            <p className="text-2xl font-bold font-mono text-primary">{totalIncome.toLocaleString()}<span className='font-sans'>₸</span></p>
          </div>
        </div>
      </div>
            ...
            <CardContent className="space-y-4 relative z-10">
              <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                ...
                <p className="text-3xl font-bold font-mono">
                  {((totalIncome * activeRule.savings) / 100).toLocaleString()}<span className='font-sans'>₸</span>
                </p>
              </div>
